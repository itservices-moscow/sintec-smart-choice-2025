  ...
  function A() {
    ...
    e.forEach(slide=>{
      ...
      [...slide.children].forEach(child=>{
        l += child.scrollHeight;
        l += isNaN(parseInt(getComputedStyle(child).marginTop)) ? 0 : parseInt(getComputedStyle(child).marginTop);
        l += isNaN(parseInt(getComputedStyle(child).marginBottom)) ? 0 : parseInt(getComputedStyle(child).marginBottom);
        l += isNaN(parseInt(getComputedStyle(child).paddingTop)) ? 0 : parseInt(getComputedStyle(child).paddingTop);
        l += isNaN(parseInt(getComputedStyle(child).paddingBottom)) ? 0 : parseInt(getComputedStyle(child).paddingBottom);
      })
      l += isNaN(parseInt(getComputedStyle(slide).paddingTop)) ? 0 : parseInt(getComputedStyle(slide).paddingTop);
      l += isNaN(parseInt(getComputedStyle(slide).paddingBottom)) ? 0 : parseInt(getComputedStyle(slide).paddingBottom);
      l > t && (t = l);
      console.log(l);
      parent = slide.closest('.slider__item');
    })
    if (parent) {
      parent.style.maxHeight = `${t}px`;
    }
    e.forEach((e) => {
        e.style.height = `${t}px`;
    });
